...
手征拉氏量一般具有如下形式
\begin{align}
   \mL_{eff} = \mL_2 + \mL_4 + \mL_4,
\end{align}
其中只包含偶数项。这主要是由于Lorentz不变性以及质量项$\sim\mO(q^2)$。

对于给定费曼图，在$p_i \mapsto t p_i$以及$m_q \mapsto t^2 m_q$变换下，若散射振幅变换如下
\begin{align}
...
\end{align}
其来源简单阐述如下，在上述标度变化下，内线的变换为$t^{n-2}$，顶点的变换为$t^{2k-n}$，由于散射矩阵的不变性，以及$S\sim\delta^n(q)M$，因此需要添加一个$n$来补偿$\delta$函数的影响，再考虑到一张图中独立圈数、内线数、顶点数之间的关系$N_L = N_I - (N_V - 1)$，即可得到上式。
