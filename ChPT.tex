...
\subsection{流代数}
   ...
   由Noether定理知道，假设场$\bm{\Phi}_i(x)$进行如下的变化
   \begin{align}
      \bm{\Phi}_i(x) \to \bm{\Phi}^{'}_i(x)=\bm{\Phi}_i(x) + \mbox{i}\delta \bm{\Phi}_i(x) = \bm{\Phi}_i(x) - \mbox{i} \epsilon_a F_{a,i}[\bm{\Phi}_i(x)]
   \end{align}
